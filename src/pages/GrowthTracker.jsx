...
const Dashboard = () => {
  ...
  const normalizedData = Object.values(policyData).map(
    (value) => (value / 10) * 100
  );
    ...
    <div className="w-full bg-[#FAFAF8]">
      <div className="ml-5 mr-5 mx-auto pt-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-7 md:mb-8">
          <h1 className="ml-3 text-[1.25rem]  font-semibold leading-normal text-[#121212] mb-4 md:mb-0">
            성장 스케치
          </h1>
        ...
        <div className="bg-white rounded-xl p-4 mb-6 shadow-lg">
          ...
          <div className="grid grid-cols-7 gap-1 md:gap-2 mb-4">
            {weekDays.map((day, index) => (
              <div
                key={index}
                className="text-center text-sm font-medium text-gray-600 py-2 md:py-3"
              >
                {day}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 gap-1 md:gap-2">
            {getDaysInMonth(currentDate).map((day, index) => (
              <div
                key={index}
                className="min-h-[45px] md:min-h-[50px] lg:min-h-[60px] flex flex-col items-center p-1 transition-colors duration-200 hover:bg-gray-50 hover:rounded-md"
              >
                {day && (
                  <>
            ...
            <div className="relative w-[200px] h-[200px] md:w-[250px] md:h-[250px] lg:w-[300px] lg:h-[300px] mx-auto">
              <svg viewBox="0 0 200 200" className="w-full h-full">
                ...
                {categories.map((_, index) => {
                  ...
                  const y = 100 + value * Math.sin((angle * Math.PI) / 180);
                  return (
                    <circle key={index} cx={x} cy={y} r="3" fill="#4CAF50" />
                  );
                })}
              </svg>
