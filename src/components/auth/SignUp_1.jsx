...
export default function SignUp_1() {
  ...
  const checkIdDuplicate = async (id) => {
    try {
      ...
      if (response.ok) {
        ...
        return { isAvailable: false, message: errorData.message || "아이디 중복 검사에 실패했습니다." };
      }
    } catch (error) {
      console.error('아이디 중복 검사 오류:', error);
      return { isAvailable: false, message: "아이디 중복 검사 중 오류가 발생했습니다." };
    }
  };
  ...
  const checkNicknameDuplicate = async (nickname) => {
    try {
      const response = await fetch(`http://localhost:8000/check-nickname/${nickname}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        return { isAvailable: false, message: errorData.message || "닉네임 중복 검사에 실패했습니다." };
      }
    } catch (error) {
      console.error('닉네임 중복 검사 오류:', error);
      return { isAvailable: false, message: "닉네임 중복 검사 중 오류가 발생했습니다." };
    }
  };
  ...
  const validateId = (id) => {
    ...
    if (id.length < 4 || id.length > 16) return "아이디는 4자 이상 16자 이하여야 합니다.";
    
    const idRegex = /^[a-zA-Z][a-zA-Z0-9_-]*$/;
    if (!idRegex.test(id)) return "아이디는 영문자로 시작하고, 영문자, 숫자, _, -만 사용 가능합니다.";
  ...
  const validatePassword = (password) => {
    ...
    if (typeCount < 3) return "영문 대문자, 소문자, 숫자, 특수문자 중 3종류 이상을 포함해야 합니다.";
    
    // 동일 문자 3회 이상 반복 금지
    if (/(.)\1{2,}/.test(password)) return "동일한 문자를 3회 이상 연속 사용할 수 없습니다.";
  ...
  const validateNickname = (nickname) => {
    ...
    if (!nicknameRegex.test(nickname)) return "닉네임은 한글, 영문, 숫자만 사용 가능합니다.";
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length === 0 || Object.values(newErrors).every(error => !error)) {
      ...
      console.log("회원가입 1단계 완료:", formData);
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <div className="h-5 flex items-center justify-end">
              {isCheckingId && (
                <p className="text-gray-500" style={{ fontSize: '0.7rem' }}>확인 중...</p>
              )}
              {!isCheckingId && errors.id && (
                <p className="text-red-500" style={{ fontSize: '0.7rem' }}>{errors.id}</p>
              )}
              {!isCheckingId && !errors.id && idCheckMessage && (
                <p className={idCheckResult === true ? "text-green-600" : "text-red-500"} style={{ fontSize: '0.7rem' }}>
                  {idCheckMessage}
                </p>
              )}
            </div>
          ...
          <div className="space-y-1">
            ...
            <div className="h-5 flex items-center justify-end">
              {errors.password && (
                <p className="text-red-500" style={{ fontSize: '0.7rem' }}>{errors.password}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="h-5 flex items-center justify-end">
              {errors.confirmPassword && (
                <p className="text-red-500" style={{ fontSize: '0.7rem' }}>{errors.confirmPassword}</p>
              )}
              {!errors.confirmPassword && formData.confirmPassword && formData.password === formData.confirmPassword && (
                <p className="text-green-600" style={{ fontSize: '0.7rem' }}>비밀번호가 일치합니다.</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="h-5 flex items-center justify-end">
              ...
              {!isCheckingNickname && errors.nickname && (
                <p className="text-red-500" style={{ fontSize: '0.7rem' }}>{errors.nickname}</p>
              )}
              {!isCheckingNickname && !errors.nickname && nicknameCheckMessage && (
                <p className={nicknameCheckResult === true ? "text-green-600" : "text-red-500"} style={{ fontSize: '0.7rem' }}>
